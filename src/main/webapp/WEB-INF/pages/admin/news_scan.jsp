...
%>
<%
	List<Tong_news>news_all = (List<Tong_news>)request.getAttribute("news_all");
...
<![endif]-->
		...
		<script type="text/javascript">
			...
			[ 
<% if(news_all!=null){
	for(int i=0;i<news_all.size();i++){
	Tong_news t = news_all.get(i);
	String content = t.getNews_source().replaceAll("	", "");
%>
{id:"<%=t.getId()%>",title:"<%=t.getNews_title()%>",source:"<%=content%>",
	time:"<%=t.getNews_time()%>",company:"<%=t.getCompany_name()%>",
	controller:"<%=t.getController()%>",category:"<%=t.getCategory()%>",
	main_service:"<%=t.getMain_service()%>", rank:"<%=t.getRank()%>",content:"<%=t.getNews_content()%>"},
<% }
}%>
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					datatype: "local",
					height: 250,
					colNames:['操作', 'ID','新闻标题','新闻来源', '新闻时间', '所属公司','控制人','行业','主营业务','营业排名','新闻内容'],
					colModel:[
						...
						{name:'title',index:'title',width:90, editable:true, sorttype:"date",unformat: pickDate},
						{name:'source',index:'source', width:90,editable: true,editoptions:{size:"20",maxlength:"30"}},
						{name:'time',index:'time', width:70, editable: true,edittype:"checkbox",editoptions: {value:"Yes:No"},unformat: aceSwitch},
						{name:'company',index:'company', width:70, editable: true,edittype:"select",editoptions:{value:"FE:FedEx;IN:InTime;TN:TNT;AR:ARAMEX"}},
						{name:'controller',index:'controller', width:60, sortable:false,editable: true,edittype:"textarea", editoptions:{rows:"2",cols:"10"}},
						{name:'category',index:'category', width:60, sorttype:"int", editable: true},
						{name:'main_service',index:'main_service', width:70, sorttype:"int", editable: true},
						{name:'rank',index:'rank', width:60, sorttype:"int", editable: true},
						{name:'content',index:'content', width:150, sorttype:"int", editable: true},
					], 
